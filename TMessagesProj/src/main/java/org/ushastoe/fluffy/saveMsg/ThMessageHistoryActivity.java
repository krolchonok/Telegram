...
package org.ushastoe.fluffy.saveMsg;

import static org.ushastoe.fluffy.helpers.MessageHelper.getFormattedTime;
...
import org.ushastoe.fluffy.saveMsg.ui.ThTextDetailCell;
...
public class ThMessageHistoryActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
    ...
    public ThMessageHistoryActivity(MessageObject messageObject) {
        Map<Long, String> map = getAccountInstance().getMessagesStorage().loadThHistory(UserConfig.getInstance(currentAccount).getCurrentUser().id, messageObject.messageOwner.dialog_id, messageObject.messageOwner.id);
        this.messageObject = messageObject;
        messageMap = new LinkedHashMap<>();
        int counter = 0;
        messageMap.put(counter++, new ThHistoryMessage(0L, Base64.encodeToString(String.valueOf(messageObject.messageText).getBytes(), Base64.DEFAULT)));
        ...
        public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            ...
            switch (viewType) {
                ...
                case 7:
                    ...
                    break;
                case 8:
                    view = new ChatMessageCell(mContext, UserConfig.selectedAccount);
                    view.setBackgroundColor(Theme.getColor(Theme.key_windowBackgroundWhite));
                    break;
                default:
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
            switch (holder.getItemViewType()) {
                case 7: {
                    ...
                    if (position >= 0 && position < messageMap.size()) {
                        ...
                        thTextDetailCell.setTextAndValue(
                                getFormattedTime(Objects.requireNonNull(messageMap.get(position)).date)
                                , new String(Base64.decode(Objects.requireNonNull(messageMap.get(position)).message, Base64.DEFAULT))
                                , false);
